<script>
    ...
    let { filterConfig, initialFilters = {} } = $props();
...
<div class="space-y-4">
    {#each Object.entries(filterConfig) as [field, config]}
        <div class="flex gap-2 items-center">
            ...
            <Select 
                ...
                on:change={(e) => handleFilterChange(field, e.target.value, activeFilters[field]?.value)}
            >
            ...
            {#if config.type === FilterType.ENUM}
                <Select 
                    ...
                    on:change={(e) => handleFilterChange(field, activeFilters[field]?.operator, e.target.value)}
                >
            ...
            {:else}
                <Input
                    ...
                    value={activeFilters[field]?.value || ''}
                    on:input={(e) => handleFilterChange(field, activeFilters[field]?.operator, e.target.value)}
                />
            {/if}
        </div>
    {/each}
    ...
    <div class="flex gap-2">
        <Button color="blue" on:click={() => dispatch('apply')}>Apply Filters</Button>
